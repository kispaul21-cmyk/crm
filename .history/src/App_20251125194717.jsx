...
const App = () => {
  ...
  // НОВОЕ: Модальное окно создания сделки
  ...
  const saveNewDeal = async () => {
    ...
    const { error } = await supabase.from('deals').insert([{
      ...
      stage: newDealData.stage, // Берем выбранный этап
    ...
    if (!error) {
      fetchDeals();
      setActiveStageId(Number(newDealData.stage)); // Переходим на выбранный этап
      setIsDealModalOpen(false); // Закрываем окно
      setNewDealData({ title: '', company: '', stage: 1 }); // Сброс
    }
  };
  ...
  async function handleFileUpload(e) {
    ...
    const newFilesData = files.map(f => ({
      name: f.name, size: (f.size / 1024 / 1024).toFixed(2) + ' MB', type: f.type, url: '#'
    }));
              ...
              {stages.map(stage => {
                ...
                return (
                  <button key={stage.id} onClick={() => { setActiveStageId(stage.id); setSelectedDeal(null); }}
                    className={`
                      relative flex items-center justify-between p-3 transition-all rounded-l-lg
                      ${isActive ? 'bg-slate-100 text-slate-900' : 'text-slate-500 hover:bg-slate-50'}
                      border-r-4 ${stage.color} /* Прямая цветная полоска справа */
                    `}
                  >
                    {!isStagesCollapsed ? (
                      <>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Название сделки</label>
                <input type="text" value={newDealData.title} onChange={e => setNewDealData({ ...newDealData, title: e.target.value })} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:border-blue-500" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Компания</label>
                <input type="text" value={newDealData.company} onChange={e => setNewDealData({ ...newDealData, company: e.target.value })} className="w-full p-2 border border-gray-200 rounded-lg outline-none focus:border-blue-500" placeholder="Название" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Этап</label>
                <select value={newDealData.stage} onChange={e => setNewDealData({ ...newDealData, stage: e.target.value })} className="w-full p-2 border border-gray-200 rounded-lg outline-none">
                  {stages.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
              </div>
              <button onClick={saveNewDeal} className="w-full bg-blue-600 text-white py-2 rounded-lg font-bold hover:bg-blue-700 mt-2">Создать</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center p-6 border-b border-gray-100">
              ...
              <button onClick={() => setIsCompanyModalOpen(false)} className="p-2 hover:bg-gray-100 rounded-full"><X size={20} className="text-gray-500" /></button>
            </div>
            <div className="p-6 grid grid-cols-2 gap-6">
              <div className="col-span-2">
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Название</label>
                <input type="text" value={editingCompany.name} onChange={e => setEditingCompany({ ...editingCompany, name: e.target.value })} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg outline-none font-medium text-lg" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">ИНН</label>
                <input type="text" value={editingCompany.inn || ''} onChange={e => setEditingCompany({ ...editingCompany, inn: e.target.value })} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg outline-none" />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Директор</label>
                <input type="text" value={editingCompany.director || ''} onChange={e => setEditingCompany({ ...editingCompany, director: e.target.value })} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg outline-none" />
              </div>
            </div>
            <div className="p-6 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
              <button onClick={() => setIsCompanyModalOpen(false)} className="px-5 py-2 text-gray-500 font-medium hover:bg-gray-200 rounded-lg">Отмена</button>
              <button onClick={saveCompany} className="px-6 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700">Сохранить</button>
            </div>
          </div>
        </div>
      )}
