...
const App = () => {
  ...
  // Fetch messages whenever a deal is selected
  useEffect(() => {
    if (selectedDeal) {
  ...
  // --- УМНАЯ ОТПРАВКА (ЗАДАЧА + СООБЩЕНИЕ) ---
  async function sendMessage() {
    ...
    if (newMessage.startsWith('#')) {
      ...
      // 2. Логика уведомления в чат
      await supabase.from('messages').insert([{
        deal_id: selectedDeal.id,
        text: `⚡ Поставлена задача: ${title}` + (subtasks.length > 0 ? ` (+${subtasks.length} подзадач)` : ''),
        is_me: true,
        reply_to_id: null
      }]);

      setNewMessage('');
      fetchTasks(); // Обновит верхнюю панель
      fetchMessages(selectedDeal.id); // Обновит чат
    } else {
      // Обычное сообщение
      await supabase.from('messages').insert([{
        deal_id: selectedDeal.id,
  ...
  const addTask = async (text, dealId = null, assignee = 'Я') => {
    if (!text) return;
    // Insert task with proper deal_id reference
    await supabase.from('tasks').insert([{ text, deal_id: dealId, assignee }]);

    // Если задача создана через мини‑поле в чате, тоже дублируем в лог
    if (dealId) {
      await supabase.from('messages').insert([{ deal_id: dealId, text: `⚡ Быстрая задача: ${text}`, is_me: true }]);
  ...
  return (
    <div className="flex h-screen w-full bg-gray-50 text-slate-800 font-sans overflow-hidden">
      ...
      {activeMenu === 'crm' && <CrmView
        ...
        chatStream={messages} currentDeals={currentDeals} dealTasks={dealTasks}
