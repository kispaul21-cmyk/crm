...
const CrmChat = ({
    ...
    // Сортировка верхних задач: сначала активные
        ...
        <div className="flex-1 flex flex-col bg-[#eef1f5] min-w-[500px]">
                        ...
                        <div key={item.id} className={`flex gap-3 group mb-2 ${item.is_me ? 'flex-row-reverse' : ''}`}>
                            ...
                            <div className="max-w-[80%]">
                                ...
                                {item.text.startsWith('⚡') ? (
                                    <div className="bg-gray-100 border border-gray-200 text-slate-600 text-xs px-4 py-2 rounded-xl inline-block">{item.text}</div>
                                ) : (
                                ...
                                {!item.text.startsWith('⚡') && <button onClick={() => setReplyTo(item)} className="text-[10px] text-gray-400 mt-1 hover:text-blue-600 opacity-0 group-hover:opacity-100">Ответить</button>}
                            </div>
                        </div>
                    );
                })}
            ...
            <div className="p-4 bg-white border-t border-gray-200">
                ...
                <div className="flex items-center gap-2 bg-gray-100 p-2 rounded-xl border border-transparent focus-within:bg-white focus-within:border-blue-200 transition-all">
                    ...
                    <textarea value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendMessage(); } }} placeholder="Сообщение или #Задача..." className="flex-1 bg-transparent outline-none text-sm resize-none max-h-24 py-2" rows={1} />
...
const TaskBubble = ({ task, toggleTask, toggleSubtask, deleteTask }) => {
    ...
    return (
        <div className="flex gap-3 mb-4 group">
            <div className="w-8 h-8 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 flex-shrink-0 shadow-sm border border-emerald-200"><ListChecks size={16} /></div>
            <div className={`max-w-[85%] bg-white border rounded-2xl rounded-tl-none shadow-sm transition-all overflow-hidden ${task.is_done ? 'opacity-60 border-gray-200' : 'border-emerald-100'}`}>
                <div className={`flex items-center p-3 gap-3 ${hasSubtasks ? 'border-b border-gray-50' : ''}`}>
                    <button onClick={() => toggleTask(task)} className={`w-5 h-5 rounded border flex-shrink-0 flex items-center justify-center transition ${task.is_done ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-gray-300 hover:border-emerald-500'}`}>{task.is_done && <CheckSquare size={12} />}</button>
                    <div className="flex-1 min-w-0"><span className={`font-bold text-sm block truncate ${task.is_done ? 'text-gray-400 line-through' : 'text-slate-700'}`}>{task.text}</span></div>
                ...
                {hasSubtasks && isExpanded && (
                    <div className="bg-emerald-50/30 p-2 space-y-1">
                        {task.subtasks.map((sub, idx) => (
                            <div key={idx} className="flex items-start gap-2 px-2 py-1.5 hover:bg-emerald-50 rounded cursor-pointer transition" onClick={() => toggleSubtask(task, idx)}>
                                <div className={`mt-0.5 w-3.5 h-3.5 rounded border flex-shrink-0 flex items-center justify-center transition ${sub.is_done || task.is_done ? 'bg-emerald-500 border-emerald-500' : 'border-gray-300 bg-white'}`}>{(sub.is_done || task.is_done) && <CheckSquare size={10} className="text-white" />}</div>
    ...
    return (
        <div className={`bg-white border rounded-lg transition-all overflow-hidden group ${task.is_done ? 'border-gray-200 opacity-60' : 'border-blue-100 shadow-sm hover:border-blue-300'}`}>
            <div className="flex items-center p-2 gap-3">
                <button onClick={() => toggleTask(task)} className={`w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center transition ${task.is_done ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300 hover:border-blue-500'}`}>{task.is_done && <CheckSquare size={10} />}</button>
                <div className="flex-1 cursor-pointer" onClick={() => hasSubtasks && setIsExpanded(!isExpanded)}><span className={`text-xs font-medium ${task.is_done ? 'text-gray-400 line-through' : 'text-slate-700'}`}>{task.text}</span></div>
                {hasSubtasks && <button onClick={() => setIsExpanded(!isExpanded)} className="text-gray-400 hover:text-blue-600 p-1">{isExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}</button>}
                <button onClick={() => deleteTask(task.id)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition p-1"><Trash2 size={12} /></button>
            </div>
            {hasSubtasks && isExpanded && (
                <div className="bg-gray-50 border-t border-gray-100 px-8 py-1 space-y-1">
                    {task.subtasks.map((sub, idx) => (
                        <div key={idx} className="flex items-center gap-2 py-1 cursor-pointer" onClick={() => toggleSubtask(task, idx)}>
                            <div className={`w-3 h-3 rounded border flex-shrink-0 flex items-center justify-center ${sub.is_done || task.is_done ? 'bg-green-500 border-green-500' : 'border-gray-300 bg-white'}`}>{(sub.is_done || task.is_done) && <CheckSquare size={8} className="text-white" />}</div>
