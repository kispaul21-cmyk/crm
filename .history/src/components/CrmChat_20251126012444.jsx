...
const CrmChat = ({
    ...
    return (
        <div className="flex-1 flex flex-col bg-[#eef1f5] min-w-[500px]">
            ...
            {/* --- ПАНЕЛЬ БЫСТРЫХ ЗАДАЧ (ЕДИНЫЙ СПИСОК) --- */}
            <div className="bg-white/90 border-b border-gray-200 px-6 py-4 backdrop-blur-sm max-h-64 overflow-y-auto custom-scrollbar shadow-sm">
                ...
                <div className="flex flex-col">
                    ...
                    {/* Поле ввода (выглядит как следующая строка списка) */}
                    <div
                        className="flex items-center gap-3 py-2 px-2 hover:bg-gray-50 rounded-lg transition group cursor-text mt-1"
                        onClick={() => document.getElementById('quick-task-input').focus()}
                    >
                        {/* Иконка плюса ровно под чекбоксами (размер w-4 h-4 как у чекбокса) */}
            ...
            <div className="space-y-1 mb-3">
                ...
                {sortedTopTasks.length === 0 && <div className="text-xs text-gray-400 italic pl-1 mb-2">Нет активных задач</div>}
            </div>

            {/* Поле ввода (Внизу списка) */}
            <div className="flex items-center gap-3 px-2 py-2 hover:bg-gray-50 rounded-lg transition group cursor-text" onClick={() => document.getElementById('quick-task-input').focus()}>
                <Plus size={16} className="text-gray-400 group-hover:text-blue-500 transition" />
                <input
                    id="quick-task-input"
                    type="text"
                    value={quickTaskText}
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-3">
        ...
        {chatStream.map(item => {
            // Если это задача в ленте чата -> показываем красивую карточку
            if (item.type === 'task') {
...
// --- КАРТОЧКА ЗАДАЧИ В ЛЕНТЕ ЧАТА (ЗЕЛЕНАЯ) ---
        ...
        <div className="flex gap-3 mb-4 group">
            ...
            <div className={`w-[80%] bg-white border rounded-xl rounded-tl-none shadow-sm overflow-hidden transition-all ${task.is_done ? 'opacity-60 border-gray-200' : 'border-emerald-100'}`}>
                <div className="flex items-center p-3 gap-3 border-b border-gray-50">
                    ...
                    {hasSubtasks && <button onClick={() => setIsExpanded(!isExpanded)} className="text-gray-400 hover:text-blue-600">{isExpanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}</button>}
                    <button onClick={() => deleteTask(task.id)} className="text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100"><Trash2 size={14} /></button>
                </div>

                {hasSubtasks && isExpanded && (
                    <div className="bg-emerald-50/20 p-2 space-y-1">
                        {task.subtasks.map((sub, idx) => (
                            <div key={idx} className="flex items-start gap-2 px-2 py-1 hover:bg-white/50 rounded cursor-pointer" onClick={() => toggleSubtask(task, idx)}>
                                <div className={`mt-0.5 w-3.5 h-3.5 rounded border flex-shrink-0 flex items-center justify-center ${sub.is_done || task.is_done ? 'bg-emerald-500 border-emerald-500' : 'border-gray-300 bg-white'}`}>
                                    {(sub.is_done || task.is_done) && <CheckSquare size={10} className="text-white" />}
                                </div>
                                <span className={`text-xs ${sub.is_done || task.is_done ? 'text-gray-400 line-through' : 'text-slate-700'}`}>{sub.text}</span>
                            </div>
                        ))}
                    </div>
                )}
...
// --- СТРОКА ЗАДАЧИ ДЛЯ ВЕРХНЕЙ ПАНЕЛИ (СПИСОК) ---
const TopPanelTask = ({ task, toggleTask, toggleSubtask, deleteTask }) => {
    const [isExpanded, setIsExpanded] = useState(false);
    const hasSubtasks = task.subtasks && task.subtasks.length > 0;

    return (
        <div className={`group border-b border-gray-50 last:border-0 hover:bg-gray-50 transition rounded-lg px-2 py-1 ${task.is_done ? 'opacity-50' : ''}`}>
            <div className="flex items-center gap-3 py-1">
                ...
                <div className="flex-1 cursor-pointer min-w-0" onClick={() => hasSubtasks && setIsExpanded(!isExpanded)}>
                    <span className={`text-sm block truncate ${task.is_done ? 'text-gray-400 line-through' : 'text-slate-700 font-medium'}`}>
                        {task.text}
                    </span>
                </div>

                {/* Стрелка (если есть подзадачи) */}
                {hasSubtasks && (
