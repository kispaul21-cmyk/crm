        ...
        <div className="flex-1 flex flex-col bg-[#eef1f5] min-w-[500px]">
            ...
            <div className="bg-white/90 border-b border-gray-200 px-6 py-4 backdrop-blur-sm max-h-64 overflow-y-auto custom-scrollbar shadow-sm">
                ...
                <div className="flex flex-col">
                    ...
                    {/* Поле ввода (как следующая строка списка) */}
                    <div
                        className="flex items-center gap-3 py-1 px-2 hover:bg-gray-50 rounded-lg transition group cursor-text mt-1"
                        onClick={() => document.getElementById('quick-task-input').focus()}
                    >
                        {/* Иконка плюса (размер w-4 h-4 как у чекбокса) */}
                        ...
                        <input
                            ...
                            className="bg-transparent text-sm w-full outline-none placeholder-gray-400 text-slate-700 font-medium"
                        />
                    </div>
                </div>
            </div>
            ...
            <div className={`w-[80%] bg-white border rounded-xl rounded-tl-none shadow-sm overflow-hidden transition-all ${task.is_done ? 'opacity-60 border-gray-200' : 'border-emerald-100'}`}>
                <div className={`flex items-center p-3 gap-3 ${hasSubtasks ? 'border-b border-gray-50' : ''}`}>
                    ...
                    <div className="flex-1 min-w-0">
                        <span className={`font-bold text-sm block truncate ${task.is_done ? 'text-gray-400 line-through' : 'text-slate-700'}`}>
                            {task.text}
                        </span>
                    </div>
                ...
                <div className="px-3 pb-1.5 text-[10px] text-gray-400 text-right">
                    {new Date(task.created_at).toLocaleTimeString().slice(0, 5)}
                </div>
            </div>
        </div>
    );
...
// --- СТРОКА ЗАДАЧИ ДЛЯ ВЕРХНЕЙ ПАНЕЛИ (ЕДИНЫЙ СПИСОК) ---
