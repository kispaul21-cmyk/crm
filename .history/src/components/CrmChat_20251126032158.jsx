...
import { MoreVertical, Plus, Smile, Send, Building2, X, CheckSquare, ChevronDown, ChevronRight, Trash2, ListChecks, Paperclip } from 'lucide-react';
            ...
            <div className="bg-white/90 border-b border-gray-200 px-6 py-4 backdrop-blur-sm max-h-64 overflow-y-auto custom-scrollbar shadow-sm">
                ...
                <div className="flex items-center gap-3 py-2 px-2 mt-1 hover:bg-gray-50 rounded-lg transition group">
                    <button
                        onClick={() => {
                            if (dealTasks.filter(t => !t.is_done).length >= 5) {
                                alert('Лимит: не более 5 активных задач.');
                                return;
                            }
                    ...
                    <input
                        ...
                        placeholder={dealTasks.filter(t => !t.is_done).length >= 5 ? 'Лимит задач исчерпан (5/5)' : 'Новый пункт...'}
                        disabled={dealTasks.filter(t => !t.is_done).length >= 5}
                ...
                {chatStream.map(item => {
                    ...
                    return (
                        <div key={item.id} className={`flex gap-3 group mb-2 ${item.is_me ? 'flex-row-reverse' : ''}`}>
                            {!item.is_me && (
                                <div className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-sm bg-purple-600">
                                    К
                                </div>
                            )}
                            <div className="max-w-[80%]">
                                ...
                                {item.text.startsWith('⚡') ? (
                                    ...
                                    <div className={`p-3 rounded-2xl text-sm shadow-sm ${item.is_me ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none'}`}>
                                        <div className="whitespace-pre-wrap">{item.text}</div>
                                        <div className={`text-[10px] text-right mt-1 ${item.is_me ? 'text-blue-100' : 'text-gray-400'}`}>
