...
const TopPanelTask = ({ task, toggleTask, toggleSubtask, deleteTask, taskFontClass, emojiMultiplier }) => {
    ...
    const hasSubtasks = task.subtasks && task.subtasks.length > 0;
    return (
        <div className={`group relative border-b border-gray-50 last:border-0 hover:bg-gray-50 transition rounded-lg px-2 ${task.is_done ? 'opacity-50' : ''}`}>
            ...
            {hasSubtasks && isExpanded && (
                <div className="absolute left-0 top-full mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-xl z-50 p-2">
                    {task.subtasks.map((sub, idx) => (
                        <div key={idx} className="flex items-center gap-2 py-1.5 cursor-pointer hover:bg-gray-50 rounded px-2 transition" onClick={() => toggleSubtask(task, idx)}>
