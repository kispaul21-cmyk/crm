                        ...
                        <div key={item.id} className={`flex gap-3 group mb-2 ${item.is_me ? 'flex-row-reverse' : ''}`}>
                            ...
                            <div className="max-w-[80%]">
                                ...
                                {item.text.startsWith('⚡') ? (
                                    <div
                                        className="bg-gray-100 border border-gray-200 text-slate-500 text-xs px-3 py-1.5 rounded-lg inline-block"
                                        style={{ fontSize: `${14 * emojiMultiplier}px` }}
                                    >
                                        {item.text}
                                    </div>
                                ) : (
                        ...
                        <div className="absolute bottom-full mb-2 left-0 bg-white border border-gray-200 rounded-lg p-3 shadow-xl z-50 grid grid-cols-8 gap-1">
                            {['😀', '😂', '😊', '😍', '🤔', '👍', '👎', '🔥', '💪', '✅', '❌', '⚡', '🎉', '💡', '📝', '📞'].map(emoji => (
                                <button
                                    ...
                                    type="button"
                                    className="text-xl"
                                    style={{ fontSize: `${24 * emojiMultiplier}px` }}
                                    onClick={() => {
...
const TaskBubble = ({ task, toggleTask, toggleSubtask, deleteTask, taskFontClass, emojiMultiplier }) => {
    ...
    return (
        <div className="flex gap-3 mb-4 group">
            ...
            <div className={`max-w-[80%] bg-emerald-50 border rounded-2xl rounded-tl-none shadow-sm overflow-hidden transition-all ${task.is_done ? 'opacity-60 border-emerald-200' : 'border-emerald-300'}`}>
                <div className={`flex items-center p-2.5 gap-2 ${hasSubtasks ? 'border-b border-emerald-100' : ''}`}>                   <button
                ...
                >
                    ...
                    <div className="flex-1 min-w-0">
                        <span className={`font-bold block truncate ${task.is_done ? 'text-gray-400 line-through' : 'text-slate-700'} ${taskFontClass}`}>{task.text}</span>
                    </div>
                    {hasSubtasks && (
                    ...
                    <button onClick={() => deleteTask(task.id)} className="text-emerald-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition p-1">
                        <Trash2 size={14} />
                    </button>
                </div>
...
const TopPanelTask = ({ task, toggleTask, toggleSubtask, deleteTask, taskFontClass, emojiMultiplier }) => {
    const [isExpanded, setIsExpanded] = useState(false);
    const hasSubtasks = task.subtasks && task.subtasks.length > 0;
    return (
        <div className={`group relative ${task.is_done ? 'opacity-50' : ''}`}>
            <div className="flex items-center gap-2 py-1.5 px-2 hover:bg-emerald-50/30 rounded-lg transition">
                <span style={{ fontSize: `${14 * emojiMultiplier}px` }}>⚡</span>
                <button
                    onClick={() => toggleTask(task)}
                    className={`w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center transition ${task.is_done ? 'bg-emerald-500 border-emerald-500 text-white' : 'border-gray-300 hover:border-emerald-500 text-transparent'}`}
                >
                    <CheckSquare size={10} />
                </button>
                <div className="flex-1 cursor-pointer min-w-0" onClick={() => hasSubtasks && setIsExpanded(!isExpanded)}>
                    <span className={`block truncate ${task.is_done ? 'text-gray-400 line-through' : 'text-slate-700 font-medium'} ${taskFontClass}`}>{task.text}</span>
                </div>
                {hasSubtasks && (
                    <button onClick={() => setIsExpanded(!isExpanded)} className="text-gray-400 hover:text-emerald-600 p-1">
            ...
            {hasSubtasks && isExpanded && (
                <div className="absolute left-0 top-full mt-1 w-full bg-white border border-gray-200 rounded-xl shadow-xl z-50 p-2">
                    {task.subtasks.map((sub, idx) => (
                        <div key={idx} className="flex items-center gap-2 py-1.5 cursor-pointer hover:bg-emerald-50 rounded px-2 transition" onClick={() => toggleSubtask(task, idx)}>
                            <div className={`w-3 h-3 rounded border flex-shrink-0 flex items-center justify-center ${sub.is_done || task.is_done ? 'bg-emerald-500 border-emerald-500' : 'border-gray-300'}`}>
                                {(sub.is_done || task.is_done) && <CheckSquare size={8} className="text-white" />}
                            </div>
                            <span className={`text-sm ${sub.is_done || task.is_done ? 'text-gray-400 line-through' : 'text-slate-600'}`}>{sub.text}</span>
                        </div>
                    ))}
                </div>
            )}
