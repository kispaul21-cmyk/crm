...
const CrmChat = ({
    ...
    const [quickTaskMessageColor, setQuickTaskMessageColor] = useState(
        localStorage.getItem('quickTaskMessageColor') || 'orange'
    );
    ...
    useEffect(() => {
        const handleColorChange = () => {
            ...
            setQuickTaskMessageColor(localStorage.getItem('quickTaskMessageColor') || 'orange');
        };
    ...
    const getQuickTaskMessageColors = () => MESSAGE_COLOR_MAP[quickTaskMessageColor] || MESSAGE_COLOR_MAP.orange;
                        ...
                        <div key={item.id} className={`flex gap-3 group mb-2 ${item.is_me ? 'flex-row-reverse' : ''}`}>
                            ...
                            <div className="max-w-[80%]">
                                ...
                                {item.text.startsWith('⚡') ? (
                                    <div className={`p-2 rounded-2xl shadow-sm ${getQuickTaskMessageColors().bg} ${getQuickTaskMessageColors().text} ${item.is_me ? 'rounded-tr-none' : 'rounded-tl-none'}`}>
                                        <div className="text-[10px] font-bold uppercase tracking-wide opacity-90 mb-1">
                                            Быстрая Задача
                                        </div>
                                        <div className={`${messageFontClass}`}>
                                            {item.text.replace('⚡', '').trim()}
                                        </div>
                                        <div className={`text-[10px] text-right mt-1 ${getQuickTaskMessageColors().timeText}`}>
                                            {new Date(item.created_at).toLocaleTimeString().slice(0, 5)}
                                        </div>
                                    </div>
                                ) : (
                                    <div className={`p-2 rounded-2xl shadow-sm ${colors.bg} ${colors.text} ${item.is_me ? 'rounded-tr-none' : 'rounded-tl-none'} ${messageFontClass}`}>
                    ...
                    {hasSubtasks && isExpanded && (
                        <div className="bg-white/50 px-2.5 py-2 space-y-1">
                            {task.subtasks.map((sub, idx) => (
                                ...
                                >
                                    <div className={`mt-0.5 w-4 h-4 rounded flex-shrink-0 flex items-center justify-center transition ${sub.is_done || task.is_done
                                        ? 'bg-emerald-500'
...
const TopPanelTask = ({ task, toggleTask, toggleSubtask, deleteTask, taskFontClass }) => {
    ...
    return (
        <div className={`group relative ${task.is_done ? 'opacity-50' : ''}`}>
            ...
            {hasSubtasks && isExpanded && (
                <div className="absolute left-0 top-full mt-1 w-full bg-white border border-emerald-200 rounded-xl shadow-xl z-50 p-2">
                    {task.subtasks.map((sub, idx) => (
                        <div
                            key={idx}
                            ...
                            onClick={() => toggleSubtask(task, idx)}
                        >
                            <div className={`w-3 h-3 rounded flex-shrink-0 flex items-center justify-center ${sub.is_done || task.is_done
                                ? 'bg-emerald-500'
                                : 'bg-white border border-emerald-300'
                                }`}>
