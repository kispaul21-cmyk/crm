...
import { MoreVertical, Smile, Send, Building2, X, CheckSquare, ChevronDown, ChevronRight, Trash2, ListChecks, Paperclip, Check, CheckCheck } from 'lucide-react';
                    ...
                    return (
                        <div key={item.id} className={`flex gap - 3 group mb - 2 ${ item.is_me ? 'flex-row-reverse' : '' } `}>
                            ...
                            <div className="max-w-[80%]">
                                ...
                                <div className={`relative px - 3 py - 2 rounded - 2xl shadow - sm ${ colors.bg } ${ colors.text } ${ item.is_me ? 'rounded-tr-none' : 'rounded-tl-none' } ${ messageFontClass } min - w - [120px]`}>
                                    <span className="whitespace-pre-wrap break-words">{displayText}</span>
                                    <span className={`float - right ml - 3 mt - 2 flex items - center gap - 1 text - [10px] ${ colors.timeText || 'opacity-70' } select - none`}>
                                        {new Date(item.created_at).toLocaleTimeString().slice(0, 5)}
                                        {item.is_me && (
                                            <CheckCheck size={12} className={colors.timeText || 'opacity-70'} />
                                        )}
                                    </span>
                                </div>
                                {!isQuickTask && (
                ...
                <div className="absolute left-0 top-full mt-1 w-full bg-white border-2 border-emerald-200 rounded-xl shadow-xl z-50 p-2">
                    {task.subtasks.map((sub, idx) => (
                        <div
                            ...
                            onClick={() => toggleSubtask(task, idx)}
                        >
                            <div className={`w - 3 h - 3 rounded border - 2 flex - shrink - 0 flex items - center justify - center ${
    sub.is_done || task.is_done
    ? 'bg-emerald-500 border-emerald-500'
    : 'border-emerald-300'
} `}>
