                        ...
                        <div key={item.id} className={`flex gap-3 group mb-2 ${item.is_me ? 'flex-row-reverse' : ''}`}>
                            ...
                            <div className="max-w-[80%]">
                                ...
                                {item.text.startsWith('⚡') ? (
                                    <div className="bg-white border border-gray-200 text-slate-600 px-4 py-3 rounded-lg shadow-sm inline-flex items-center gap-2 w-full">
                                        <span style={{ fontSize: `${18 * emojiMultiplier}px` }}>⚡</span>
                                        {/* Убираем слово "Поставленная" из отображения */}
                                        <span className="text-xs font-medium">
                                            {item.text.slice(1).replace('Поставлена задача:', 'Задача:').trim()}
                                        </span>
                                    </div>
                                ) : (
