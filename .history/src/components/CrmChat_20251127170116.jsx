...
const CrmChat = ({
    ...
    // Helper to parse system messages with subtasks
    const parseSystemMessage = (text) => {
        // Убираем префикс "⚡ "
        const cleanText = text.replace(/^⚡\s*/, '');

        // Проверяем, есть ли информация о подзадачах в скобках (например, "(+2 подзадач)")
        // Если текст формируется в App.jsx, он может содержать переводы строк или просто счетчик
        // В текущей реализации в App.jsx подзадачи не сохраняются в текст сообщения явно, только счетчик.
        // Чтобы показать их здесь, нужно либо менять App.jsx, либо искать соответствующую задачу в dealTasks.

        // ПОДХОД 1: Ищем задачу в списке dealTasks, которая соответствует названию
        // Извлекаем название задачи (все до открывающей скобки с подзадачами, если есть)
        ...
        // Пытаемся найти эту задачу в списке задач сделки, чтобы взять реальные подзадачи
        // Это эвристика, так как ID задачи не хранится в сообщении
    ...
    return (
        <div className="flex-1 flex flex-col bg-[#eef1f5] min-w-[500px]">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-3">
                ...
                {chatStream.map(item => {
                    // Рендер КАРТОЧКИ ЗАДАЧИ (интерактивной)
                    if (item.type === 'task') {
                        return (
                            <TaskBubble
                                key={item.id}
                    ...
                    // Определение цветов для сообщений
                    ...
                    // Рендер СИСТЕМНОГО СООБЩЕНИЯ (Молния)
                        ...
                        return (
                            <div key={item.id} className="flex gap-3 mb-2 w-full">
                                <div className="max-w-[85%] w-full">
                                    <div className="bg-white border border-gray-200 text-slate-600 px-4 py-3 rounded-lg shadow-sm flex flex-col gap-2">
                                        ...
                                        {/* Заголовок задачи */}
                                        <div className="text-sm font-medium text-slate-800 ml-7">
                                            {title}
                                        </div>
                ...
                <div className={`bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden transition-all ${task.is_done ? 'opacity-60' : ''}`}>
                    ...
                    <div className={`flex items-center p-3 gap-3 ${hasSubtasks ? 'border-b border-gray-100' : ''}`}>
                        <button
                            ...
                            className={`w-5 h-5 rounded border flex-shrink-0 flex items-center justify-center transition ${task.is_done
                    ...
                    {hasSubtasks && isExpanded && (
                        <div className="bg-gray-50/50 px-3 py-2 space-y-1">
                            {task.subtasks.map((sub, idx) => (
                                ...
                                >
                                    <div className={`mt-0.5 w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center transition ${sub.is_done || task.is_done
...
const TopPanelTask = ({ task, toggleTask, toggleSubtask, deleteTask, taskFontClass }) => {
    ...
    return (
        <div className={`group relative ${task.is_done ? 'opacity-50' : ''}`}>
            <div className="flex items-center gap-2 py-1.5 px-2 hover:bg-gray-100 rounded-lg transition">
                <button
                    onClick={() => toggleTask(task)}
