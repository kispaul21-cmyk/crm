                        ...
                        <div key={item.id} className={`flex gap-3 group mb-2 ${item.is_me ? 'flex-row-reverse' : ''}`}>
                            ...
                            <div className="max-w-[80%]">
                                ...
                                {item.text.startsWith('⚡') ? (
                                    ...
                                    <div 
                                        className={`p-3 rounded-2xl shadow-sm ${colors.bg} ${colors.text} ${item.is_me ? 'rounded-tr-none' : 'rounded-tl-none'} ${messageFontClass} cursor-context-menu`}
                                        onContextMenu={(e) => {
                                            e.preventDefault();
                                            const menu = document.createElement('div');
                                            menu.className = 'fixed bg-white border border-gray-200 rounded-lg shadow-xl z-[100] py-1 min-w-[140px]';
                                            ...
                                            buttons.filter(btn => btn.show).forEach(btn => {
                                                ...
                                                button.className = 'w-full text-left px-3 py-1.5 hover:bg-gray-100 text-xs text-gray-700 transition';
