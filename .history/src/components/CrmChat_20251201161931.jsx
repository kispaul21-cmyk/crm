...
const CrmChat = ({
    ...
    return (
        <div className="flex-1 flex flex-col bg-[#eef1f5] overflow-hidden relative">
            ...
            <div
                className="bg-white border-b border-gray-100 flex-shrink-0 cursor-pointer hover:bg-gray-50 transition"
            ...
            {/* Quick Tasks Panel - ФИКСИРОВАННЫЙ */}
            <div className="bg-white border-b border-gray-100 flex-shrink-0 max-h-[30vh] overflow-y-auto">

                {/* Модальное окно редактирования задачи */}
                {editingTask && (
                    <div className="mx-4 mt-3 mb-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            ...
            <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-6 space-y-3" onClick={() => selectedMessages.length > 0 && setSelectedMessages([])}>
                ...
                {chatStream.map(item => {
                    ...
                    return (
                        <div key={item.id} className={`flex gap-2 group mb-2 ${item.is_me ? 'flex-row-reverse' : ''}`}>
                            {!item.is_me && (
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-sm flex-shrink-0 ${incomingColors.bg}`}>
                                    К
                                </div>
                            )}
                            <div className="max-w-[70%] min-w-0">
                                ...
                                {item.text.startsWith('⚡') ? (
                                    ...
                                    <div
                                        className={`px-3 py-2 rounded-xl shadow-sm transition-all duration-150 hover:shadow-md ${colors.bg} ${colors.text} ${item.is_me ? 'rounded-tr-sm' : 'rounded-tl-sm'} ${messageFontClass} ${selectedMessages.includes(item.id) ? 'ring-2 ring-blue-500' : ''} cursor-context-menu break-words overflow-wrap-anywhere`}
                                        onClick={(e) => {
