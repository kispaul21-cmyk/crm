...
const StageButton = ({ stage, dealCount, isActive, onClick }) => {
  // Теперь снова используем stage.color как CSS-класс.
  // Добавляем `border-r-4` для установки толщины рамки.
  const buttonClasses = `relative flex items-center justify-between w-full p-3 transition-all rounded-l-lg text-left border-r-4 ${
    isActive 
      ? 'bg-slate-100 text-slate-900' 
      : 'text-slate-500 hover:bg-slate-50'
  } ${stage.color || 'border-gray-200'}`; // Если цвета нет, рамка серая
...
const CrmStages = ({
  ...
  return (
    <div className={`${isCollapsed ? 'w-16' : 'w-64'} bg-white border-r border-gray-200 flex flex-col transition-all duration-300 flex-shrink-0 relative z-20`}>
      ...
      {/* Список этапов */}
      <div className="flex-1 overflow-y-auto py-2 flex col flex-col justify-start space-y-1 px-2">
        {filteredStages.map(stage => {
          // --- ОТЛАДКА ---
          console.log(`Этап: "${stage.name}", Цвет из БД: "${stage.color}"`);
          // --- КОНЕЦ ОТЛАДКИ ---
