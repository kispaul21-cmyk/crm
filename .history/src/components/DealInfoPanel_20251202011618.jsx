...
const DealInfoPanel = ({
    ...
    const handleSave = async () => {
        ...
        // Если компании всё ещё нет, но в форме есть ИНН (и пользователь не отметил "Нет ИНН"),
        // попробуем найти компанию по ИНН или создать минимальную запись в `companies`.
        if (!companyId && formData.inn && !formData.noInn) {
            try {
                const { data: foundByInn, error: findErr } = await supabase
                    .from('companies')
                    .select('*')
                    .eq('inn', formData.inn)
                    .maybeSingle();
                ...
                if (findErr) {
                    console.warn('Error searching company by INN:', findErr);
                }

                if (foundByInn) {
                    companyId = foundByInn.id;
                } else {
                    // Build minimal company payload: prefer DaData full info if available
                    const newCompanyPayload = dadataCompany && dadataCompany.inn === String(formData.inn)
                        ? {
                            name: dadataCompany.name || (formData.company || `Компания ${formData.inn}`),
                            inn: dadataCompany.inn,
                            director: dadataCompany.director || null,
                            phone: dadataCompany.phone || null,
                            email: dadataCompany.email || null
                        }
                        : {
                            name: formData.company || `Компания ${formData.inn}`,
                            inn: formData.inn
                        };

                    const { data: createdByInn, error: createByInnErr } = await supabase
                        .from('companies')
                    ...
                    if (createByInnErr) {
                        console.warn('Error creating company by INN:', createByInnErr);
                    }

                    if (createdByInn) {
                        companyId = createdByInn.id;
                    }
                }
            } catch (err) {
                console.error('Unexpected error while finding/creating company by INN:', err);
            }
        }
        ...
        if (rawValue !== undefined && rawValue !== null && rawValue !== '') {
            numericValue = parseFloat(String(rawValue).replace(/\s/g, ''));
            if (Number.isNaN(numericValue)) numericValue = null;
        }

        // Determine which INN to send: prefer company's INN if companyId found,
        // otherwise use formData.inn (unless user marked noInn).
        let innToSend = undefined;
        if (formData.noInn) {
            innToSend = undefined;
        } else if (companyId) {
            const foundCompany = companies.find(c => c.id === companyId);
            if (foundCompany && foundCompany.inn) innToSend = foundCompany.inn;
            else if (formData.inn) innToSend = formData.inn;
        } else if (formData.inn) {
            innToSend = formData.inn;
        }
        ...
        // Debug: логируем полезную информацию перед запросом
        console.debug('Attempting to save deal, payload:', dealData);

        // Убираем поля с null/undefined и пустые строки чтобы не отправлять их в PostgREST
        // Также, если пользователь отметил, что нет ИНН, не отправляем поле `inn` вовсе.
        if (formData.noInn) {
            dealData.inn = undefined;
        }
        ...
        const cleanedDealData = cleanPayload(dealData);
        console.debug('Cleaned payload (null/undefined/empty removed):', cleanedDealData);

        try {
            if (deal?.id) {
                // === UPDATE === (возвращаем обновлённую строку для диагностики)
                const { data: updatedDeal, error } = await supabase
                    .from('deals')
                    .update(cleanedDealData)
                    .eq('id', deal.id)
                    .select('*')
                    .single();

                console.debug('Update response:', { updatedDeal, error });
                ...
                // === CREATE === (возвращаем созданную строку для диагностики)
                const { data: createdDeal, error } = await supabase
                    .from('deals')
                    .insert([cleanedDealData])
                    .select('*')
                    .single();

                console.debug('Insert response:', { createdDeal, error });

                if (!error) {
                    onClose();
                    if (onDealCreated) onDealCreated();
                } else {
                    alert('Ошибка создания: ' + (error.message || JSON.stringify(error)));
                }
            }
        } catch (err) {
            console.error('Unexpected error when saving deal:', err);
            alert('Непредвиденная ошибка при сохранении сделки. Проверьте консоль разработчика.');
        }
    };
