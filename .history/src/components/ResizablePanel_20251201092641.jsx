...
const ResizablePanel = ({
  ...
  className = '',
  isCollapsed = false, // Новый проп для свернутого состояния
  collapsedWidth = 64 // Ширина в свернутом состоянии
}) => {
  ...
  // Вычисляем актуальную ширину с учетом collapsed состояния
  ...
  return (
    <div
      ref={panelRef}
      className={`flex-shrink-0 ${!isResizing ? 'transition-all duration-300' : ''} ${className}`}
      style={{ width: `${actualWidth}px`, position: 'relative' }}
    >
      {children}

      {/* Ресайзер - вертикальная полоска для перетаскивания (скрыт когда свернуто) */}
      {!isCollapsed && (
        <div
          ...
          style={{
            width: '8px',
            zIndex: 9999,
            marginLeft: side === 'right' ? '0' : '-4px',
            marginRight: side === 'left' ? '0' : '-4px'
          }}
          onMouseDown={handleMouseDown}
        >
          {/* Видимая линия при hover */}
          <div
            className="absolute inset-0 bg-transparent group-hover:bg-blue-500 transition-colors"
