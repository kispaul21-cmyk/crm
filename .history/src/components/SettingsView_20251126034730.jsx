...
const SettingsView = ({ stages, onStagesChange }) => {
    ...
    const applyPresetGradient = async (gradientColors) => {
        ...
        const updated = localStages.map((s, i) => {
            ...
            const hexColor = gradientColors[colorIndex];

            // Найти или создать цвет в customColors
            let colorId = customColors.find(c => c.hex === hexColor)?.id;
            if (!colorId) {
                // Если цвета нет, используем ближайший из палитры
                colorId = customColors[Math.floor((i / (total - 1 || 1)) * (customColors.length - 1))]?.id || customColors[0].id;
            }

            return { ...s, color: colorId };
        });
