...
const SettingsView = ({ stages, onStagesChange }) => {
    ...
    useEffect(() => {
        localStorage.setItem('quickTaskColor', quickTaskColor);
        window.dispatchEvent(new CustomEvent('messageColorsChanged'));
    }, [quickTaskColor]);

    // Save font size settings
    useEffect(() => {
        localStorage.setItem('crmGlobalFontSize', crmGlobalFontSize);
        window.dispatchEvent(new CustomEvent('fontSizeChanged'));
    }, [crmGlobalFontSize]);

    useEffect(() => {
        localStorage.setItem('crmApplySidebar', crmApplySidebar);
        window.dispatchEvent(new CustomEvent('fontSizeChanged'));
    }, [crmApplySidebar]);

    useEffect(() => {
        localStorage.setItem('crmApplyPipeline', crmApplyPipeline);
        window.dispatchEvent(new CustomEvent('fontSizeChanged'));
    }, [crmApplyPipeline]);

    useEffect(() => {
        localStorage.setItem('crmApplyRightPanel', crmApplyRightPanel);
        window.dispatchEvent(new CustomEvent('fontSizeChanged'));
    }, [crmApplyRightPanel]);

    useEffect(() => {
        localStorage.setItem('crmApplyChatHeader', crmApplyChatHeader);
        window.dispatchEvent(new CustomEvent('fontSizeChanged'));
    }, [crmApplyChatHeader]);

    useEffect(() => {
        localStorage.setItem('chatIncomingFontSize', chatIncomingFontSize);
        window.dispatchEvent(new CustomEvent('fontSizeChanged'));
    }, [chatIncomingFontSize]);

    useEffect(() => {
        localStorage.setItem('chatOutgoingFontSize', chatOutgoingFontSize);
        window.dispatchEvent(new CustomEvent('fontSizeChanged'));
    }, [chatOutgoingFontSize]);

    useEffect(() => {
        localStorage.setItem('chatEmojiSize', chatEmojiSize);
        window.dispatchEvent(new CustomEvent('fontSizeChanged'));
    }, [chatEmojiSize]);
                ...
                {activeSection === 'crm' && (
                    <div className="space-y-6">
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                            ...
                            <div className="mb-6">
                                ...
                                <div className="flex gap-2">
                                    {Object.entries(FONT_SIZES).map(([key, size]) => (
                                        <button
                                            key={key}
                                            onClick={() => setCrmGlobalFontSize(key)}
                                            className={`flex-1 px-4 py-3 rounded-xl font-bold transition-all ${crmGlobalFontSize === key
                                ...
                                <p className="text-xs text-gray-400 mt-2">
                                    {FONT_SIZES[crmGlobalFontSize]?.description}
                                </p>
                            </div>
                            ...
                            <div className="space-y-3">
                                ...
                                <label className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl cursor-pointer hover:bg-gray-100 transition">
                                    <input
                                        type="checkbox"
                                        checked={crmApplySidebar}
                                        onChange={(e) => setCrmApplySidebar(e.target.checked)}
                                        className="w-5 h-5 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                                    />
                                    <span className="flex-1 font-medium text-slate-700">Боковое меню (Sidebar)</span>
                                </label>

                                <label className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl cursor-pointer hover:bg-gray-100 transition">
                                    <input
                                        type="checkbox"
                                        checked={crmApplyPipeline}
                                        onChange={(e) => setCrmApplyPipeline(e.target.checked)}
                                        className="w-5 h-5 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                                    />
                                    <span className="flex-1 font-medium text-slate-700">Воронка этапов и список сделок</span>
                                </label>

                                <label className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl cursor-pointer hover:bg-gray-100 transition">
                                    <input
                                        type="checkbox"
                                        checked={crmApplyRightPanel}
                                        onChange={(e) => setCrmApplyRightPanel(e.target.checked)}
                                        className="w-5 h-5 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                                    />
                                    <span className="flex-1 font-medium text-slate-700">Правая панель (инфо о сделке)</span>
                                </label>

                                <label className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl cursor-pointer hover:bg-gray-100 transition">
                                    <input
                                        type="checkbox"
                                        checked={crmApplyChatHeader}
                                        onChange={(e) => setCrmApplyChatHeader(e.target.checked)}
                                        className="w-5 h-5 text-blue-600 rounded focus:ring-2 focus:ring-blue-500"
                                    />
                                    <span className="flex-1 font-medium text-slate-700">Шапка чата и быстрые задачи</span>
                                </label>
                            </div>
                        </div>

                        {/* НОВЫЙ БЛОК 2: Сообщения в чате */}
                        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                            <h3 className="text-sm font-bold text-gray-400 uppercase mb-4 flex items-center gap-2">
                            ...
                            <div className="mb-6">
                                ...
                                <div className="flex gap-2">
                                    {Object.entries(FONT_SIZES).map(([key, size]) => (
                                        <button
                                            key={key}
                                            onClick={() => setChatIncomingFontSize(key)}
                                            className={`flex-1 px-4 py-3 rounded-xl font-bold transition-all ${chatIncomingFontSize === key
                            ...
                            <div className="mb-6">
                                ...
                                <div className="flex gap-2">
                                    {Object.entries(FONT_SIZES).map(([key, size]) => (
                                        <button
                                            key={key}
                                            onClick={() => setChatOutgoingFontSize(key)}
                                            className={`flex-1 px-4 py-3 rounded-xl font-bold transition-all ${chatOutgoingFontSize === key
                                                ? 'bg-blue-600 text-white shadow-lg'
                                                : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                                                }`}
                                            style={{ fontSize: `${size.value}px` }}
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-600 mb-3">
                                    Размер эмодзи и символов:
                                </label>
                                <div className="flex gap-2">
                                    {Object.entries(FONT_SIZES).map(([key, size]) => (
                                        <button
                                            key={key}
                                            onClick={() => setChatEmojiSize(key)}
                                            className={`flex-1 px-4 py-3 rounded-xl font-bold transition-all ${chatEmojiSize === key
                                                ? 'bg-green-600 text-white shadow-lg'
                                                : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                                                }`}
                                            style={{ fontSize: `${size.value * 1.2}px` }}
                                        >
                                            😊
                                        </button>
                                    ))}
                        ...
                        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                            ...
                            {/* Исходящие сообщения */}
                            <div className="mb-6">
                                <label className="block text-xs font-bold text-gray-600 mb-3">Цвет исходящих сообщений:</label>
                                <div className="flex gap-2 mb-4 flex-wrap">
                                    {MESSAGE_COLORS.map(color => (
                                        <button
                                            key={color.id}
                                            onClick={() => setMyMessageColor(color.id)}
                                            className={`w-10 h-10 rounded-full ${color.bg} transition-all hover:scale-110 ${myMessageColor === color.id ? 'ring-4 ring-offset-2 ring-blue-400 scale-110' : ''
                                                }`}
                                            title={color.name}
                                        />
                                    ))}
                                ...
                                <div className="flex justify-end">
                                    <div className={`${MESSAGE_COLORS.find(c => c.id === myMessageColor)?.bg} ${MESSAGE_COLORS.find(c => c.id === myMessageColor)?.text} px-4 py-2 rounded-2xl rounded-tr-none text-sm shadow-sm`}>
                                        Пример сообщения
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Входящие сообщения */}
                            <div className="mb-6">
                                <label className="block text-xs font-bold text-gray-600 mb-3">Цвет входящих сообщений:</label>
                                <div className="flex gap-2 mb-4 flex-wrap">
                                    {MESSAGE_COLORS.map(color => (
                                        <button
                                            key={color.id}
                                            onClick={() => setIncomingMessageColor(color.id)}
                                            className={`w-10 h-10 rounded-full ${color.bg} transition-all hover:scale-110 ${incomingMessageColor === color.id ? 'ring-4 ring-offset-2 ring-purple-400 scale-110' : ''
                                                }`}
                                            title={color.name}
                                        />
                                    ))}
                                ...
                                <div className="flex justify-start">
                                    <div className={`${MESSAGE_COLORS.find(c => c.id === incomingMessageColor)?.bg} ${MESSAGE_COLORS.find(c => c.id === incomingMessageColor)?.text} px-4 py-2 rounded-2xl rounded-tl-none text-sm shadow-sm`}>
                                        Пример сообщения
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Быстрые задачи */}
                            <div>
                                <label className="block text-xs font-bold text-gray-600 mb-3">Цвет быстрых задач:</label>
                                <div className="flex gap-2 mb-4 flex-wrap">
                                    {MESSAGE_COLORS.map(color => (
                                        <button
                                            key={color.id}
                                            onClick={() => setQuickTaskColor(color.id)}
                                            className={`w-10 h-10 rounded-full ${color.bg} transition-all hover:scale-110 ${quickTaskColor === color.id ? 'ring-4 ring-offset-2 ring-orange-400 scale-110' : ''
                                                }`}
                                            title={color.name}
                                        />
                                    ))}
                                ...
                                <div className="flex justify-end">
                                    <div>
                                        ...
                                        <div className={`${MESSAGE_COLORS.find(c => c.id === quickTaskColor)?.bg} ${MESSAGE_COLORS.find(c => c.id === quickTaskColor)?.text} px-4 py-2 rounded-2xl rounded-tr-none text-sm shadow-sm`}>
                                            Пример быстрой задачи
                                        </div>
                                    </div>
                                </div>
                            </div>
...
const AnalyticsSettings = ({ analyticsPeriod, setAnalyticsPeriod }) => (
    <div className="space-y-6">
        <div>
            ...
            <p className="text-sm text-gray-500">Параметры отображения отчётов</p>
        </div>
...
const IntegrationsSettings = ({ onOpenModal }) => (
    <div className="space-y-6">
        <div>
            ...
            <p className="text-sm text-gray-500">Подключение внешних сервисов</p>
        </div>

        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm space-y-4">
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white rounded-lg flex items-center justify-center shadow-sm text-2xl font-bold text-orange-500">
                        D
                    </div>
                    <div>
                        <h3 className="font-bold text-slate-800">DaData</h3>
                        <p className="text-xs text-gray-500">Автозаполнение реквизитов компаний</p>
                    </div>
                </div>
                <button
                    onClick={onOpenModal}
            ...
            <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200 opacity-60">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white rounded-lg flex items-center justify-center shadow-sm text-2xl font-bold text-blue-500">
                        T
                    </div>
                    <div>
                        <h3 className="font-bold text-slate-800">Telegram</h3>
                        <p className="text-xs text-gray-500">Уведомления и боты (Скоро)</p>
                    </div>
                </div>
                <button disabled className="px-4 py-2 bg-gray-100 border border-gray-200 rounded-lg text-sm font-medium text-gray-400 cursor-not-allowed">
                    Скоро
                </button>
            </div>
        </div>
