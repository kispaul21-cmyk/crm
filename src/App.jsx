...
const App = () => {
  ...
  const deleteCompany = async (companyId) => {
    ...
    setIsLoading(true);
    try {
      // Сначала обнуляем ссылки в сделках, чтобы не нарушить FK
      ...
      if (updateError) {
        console.warn('Ошибка обнуления company_id у сделок:', updateError);
        alert('Не удалось удалить компанию: проблема с обновлением связанных сделок. Подробнее в консоли.');
        return;
      }
      ...
      alert('Ошибка при удалении компании. Посмотрите консоль.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const deleteDeal = async (dealId) => {
    ...
    try {
      ...
      if (error) {
        alert('Ошибка удаления сделки: ' + (error.message || JSON.stringify(error)));
      } else {
        setDeals(prev => prev.filter(d => d.id !== dealId));
        if (selectedDeal && selectedDeal.id === dealId) setSelectedDeal(null);
      }
    } catch (err) {
      console.error('Unexpected error deleting deal:', err);
      alert('Ошибка при удалении сделки. Посмотрите консоль.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const updateStage = async (sid) => {
    ...
    fetchDeals();
    // Не закрываем панель, оставляем выбранную сделку открытой,
    // но обновляем её этап в состоянии, чтобы UI отобразил изменение.
    setSelectedDeal(prev => prev ? ({ ...prev, stage: sid }) : prev);
    setActiveStageId(Number(sid));
  };

  const addStage = async (name, color) => {
    const nextPosition = Math.max(...stages.map(s => s.position || 0), 0) + 1;
    await supabase.from('stages').insert([{ name, color, position: nextPosition }]);
    fetchStages();
  };

  const deleteStage = async (id) => {
    // 1. Проверяем, есть ли сделки в этом этапе
    const { data: dealsInStage, error: checkError } = await supabase
      .from('deals')
      .select('id')
      .eq('stage', id)
      .limit(1);
    ...
    if (checkError) {
      console.error('Ошибка при проверке сделок в этапе:', checkError);
      alert('Не удалось проверить наличие сделок в этапе. Попробуйте снова.');
      return;
    }

    if (dealsInStage && dealsInStage.length > 0) {
      alert('Нельзя удалить этап, в котором есть сделки. Сначала переместите их в другой этап.');
      return;
    }

    // 2. Если сделок нет, удаляем этап
  ...
  const reorderStages = async (newOrder) => {
    // Оптимистичное обновление UI для плавности
    setStages(newOrder);
    ...
    try {
      ...
      console.error("Ошибка при изменении порядка этапов:", error);
      // В случае ошибки, откатываемся к состоянию из БД
      fetchStages();
    }
  };
